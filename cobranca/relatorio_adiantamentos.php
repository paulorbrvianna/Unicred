...
$por_pagina = 30; 
$selec2 = mysql_query ("SELECT ad.cc, analise_ger.risco, ad.nome, cad_completo.dddcel, cad_completo.cel, ad.dias_ad, ad.data_ad, ad.gerente, ad.adto FROM saldocc  LEFT JOIN ad ON(ad.cc = saldocc.cc) LEFT JOIN cad_completo ON(cad_completo.cpf = saldocc.cpf) LEFT JOIN analise_ger ON(cad_completo.cpf = analise_ger.cpf)  WHERE (adto > 0) AND (dias_ad BETWEEN $dia1 AND $dia2) AND saldocc.codger IN ($ids)  ORDER BY dias_ad ASC ");
//SELECT * FROM ad, cad_completo WHERE (adto > 0) AND (dias_ad BETWEEN $dia1 AND $dia2) AND cad_completo.codgerente IN ($ids) AND (ad.cc = cad_completo.conta) ORDER BY dias_ad 
...
$pdf->SetX("2.5");
$pdf->Cell(0, 0, substr($resul2['nome'], 0, 40), 0, 1, 'L');
$pdf->SetX("11");
$pdf->Cell(0, 0, $resul2['risco'], 0, 1, 'L');
$pdf->SetX("12.5");
$pdf->Cell(0, 0,'( '.$resul2['dddcel'].' )  '.$resul2['cel'], 0, 1, 'L');
$pdf->SetX("15.3");
...
$pdf->Output('relatorios/RelatorioAD'.'_'.$tc.'_'.$usuario.'_'.$data_registro.'_'.$hora_registro.'.pdf');
